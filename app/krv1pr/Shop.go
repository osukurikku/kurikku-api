...
func GetShopItems(md common.MethodData) common.CodeMessager {
	var arrayItems []Item
	var user User
	err := md.DB.QueryRow(`SELECT users.id, users.username, users.privileges, users.donor_expire, users.balance FROM users WHERE users.id = ?`, md.ID()).Scan(&user.UserID, &user.Username, &user.Privileges, &user.DonorExpire, &user.Balance)
